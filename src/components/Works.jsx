  ...
  return (
    <div
      ...
      <div style={{ width: '100%', aspectRatio: work.type === 'meme' ? '4/5' : '1/1', background: work.bg, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 44, position: 'relative', overflow: 'hidden' }}>
        {work.image
          ? <img src={work.image} alt={work.name} style={{ width: '100%', height: '100%', objectFit: 'cover', display: 'block', transform: hovered ? 'scale(1.05)' : 'scale(1)', transition: 'transform .4s cubic-bezier(.16,1,.3,1)' }} />
          : <span>{work.icon}</span>
        }
